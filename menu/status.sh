...
for svc in  udp-custom dnstt zivpn xray stunnel5 ohp proxy.service edu.service; do
systemctl restart $svc >/dev/null 2>&1
echo -e "${LN}┃${NC} Restarted: $svc"
done
...
echo -e "${LN}┃${NC} WS HTTPS                   : $(cek_service edu.service)"
echo -e "${LN}┃${NC} WS HTTP                    : $(cek_service proxy.service)"

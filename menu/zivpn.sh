...
if grep -qw "$user" /etc/zivpn/user.txt; then
...
if grep -qw "$pass" /etc/zivpn/user.txt || grep -qw "\"$pass\"" /etc/zivpn/config.json; then
...
echo "$user $pass $exp" >> /etc/zivpn/user.txt
...
expiry=$(echo "$line" | awk '{print $3}' | tr -d '\r')
printf "${LN}┃ %-5s %-15s %-15s %-10s ${NC}\n" "$i" "$username" "$password" "$expiry"
((i++))
done < /etc/zivpn/user.txt
echo -e "${LN}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${NC}"
echo -e "${LN}●━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━●${NC}"
echo ""
...
line=$(awk -v u="$user" '$1==u {print; exit}' /etc/zivpn/user.txt)
if [[ -z "$line" ]]; then
...
sed -i "/^$user /d" /etc/zivpn/user.txt
systemctl restart zivpn
...
current_exp=$(echo "$line" | awk '{print $3}' | tr -d '\r')
...
password=$(echo "$line" | awk '{print $2}')
sed -i "/^$user /c\\$user $password $new_exp" /etc/zivpn/user.txt
clear
echo -e "${LN}┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓${NC}"
